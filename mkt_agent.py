...
def conversion_rate_last_7_days(df, platform):
    ...
    if platform == "Meta Ads":
        taxa_ultimos_7_dias = df_ultimos_7_dias['Compras'].sum() / df_ultimos_7_dias['Visualizações da Página de Destino'].sum() if df_ultimos_7_dias['Visualizações da Página de Destino'].sum() > 0 else 0
        taxa_periodo_anterior = df_periodo_anterior['Compras'].sum() / df_periodo_anterior['Visualizações da Página de Destino'].sum() if df_periodo_anterior['Visualizações da Página de Destino'].sum() > 0 else 0
    elif platform == "Google Ads":
...
if meta_ads_df is not None and google_ads_df is not None:
    ...
    if df.empty:
        ...
        with col1:
            ...
            st.metric(f"Conversão 7 Dias ({plataforma})", f"{taxa_ultimos_7_dias:.2%} (vs {taxa_periodo_anterior:.2%})", status_conversao)
        with col2:
